...
export interface CoverflowCarouselProps {
  ...
  gap?: number;
  loop?: boolean;
  /** How much bigger the centre card gets, as a fraction (0.2 = 20% larger). */
  centerScale?: number;
  /** Advances on its own every N ms. Zero disables it. */
  autoplayMs?: number;
  showCaption?: boolean;
...
export function CoverflowCarousel({
  ...
  const paint = React.useCallback(() => {
    ...
    cardRefs.current.forEach((card, index) => {
      ...
      // 1 exactly at the centre, 0 once a full card out — so the growth
      // tracks the drag continuously instead of snapping on settle.
      ...
      // A imagem do card em foco aparece inteira (contain); as laterais
      // continuam preenchendo o card (cover) pra grade não ficar furada.
      const image = card.querySelector("img");
      if (image) {
        image.style.objectFit = focus > 0.5 ? "contain" : "cover";
      }
  ...
  // Passa sozinho até a pessoa usar as setas — aí para de vez, porque a
  // partir do momento em que ela assume o controle, continuar avançando
  // sozinho tira o card que ela escolheu da frente.
    ...
    >
      <div className="relative">
        <div
          ...
          // Padding vertical generoso: além das sombras, o card central cresce
          // (centerScale) e precisa de folga pra não ser cortado pelo overflow.
        ...
        >
          <div
            ...
            style={{
              // 4:5 — a altura acompanha a largura do card, senão o card
              // vertical estoura o frame que era quadrado.
