...
export default function KineticGrid({
  ...
  // ── Animation loop ──────────────────────────────────────────────────────────
  // animateRef sidesteps a self-reference inside its own useCallback: the
  // rAF callback needs to schedule the *next* animate call, but reading the
  // `animate` const inside its own initializer is flagged (and fragile) even
  // though it works today, since it depends on the closure resolving after
  // the const is assigned.
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      // Tamanho do próprio container (não da viewport inteira): o canvas é
      // absolute dentro dele agora, e a seção pode ficar mais alta que uma
      // tela (título + carrossel), então window.innerHeight sozinho deixava
      // o canvas com resolução menor que a caixa e esticava o traçado.
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        // absolute (não fixed): o componente original assumia que era o
        // fundo da PÁGINA inteira. Aqui ele é só o fundo de UMA seção da
        // home — com fixed, o canvas ficava grudado na viewport e vazava
        // por cima das seções seguintes até algo opaco cobrir. absolute
        // prende o canvas ao container relative do próprio componente, que
        // já tem overflow-hidden, então ele rola embora com a seção.
